...
19. Do NOT use NO_MAP_WEAK_FIT for answers that are short but directly and clearly respond to the question. A brief answer such as "Not that I remember" or "I don't have any experience with that" is a complete, valid response — the user may genuinely have nothing else to share. Only use NO_MAP_WEAK_FIT when the answer lacks substance, is off-topic, or is unclear, not when it is simply brief.
...
RESPOND ONLY with the text of the new question. Keep it to ONE short, clear sentence in plain, conversational language — typically 12-20 words, never wordy or run-on. Only add a brief, casual acknowledgment (a few words, like “Nice work!”) if it feels natural; otherwise skip it. If you include one, end it with a period so the question begins as a separate sentence. Do not include any other text, explanation, or formatting.`;
  }
